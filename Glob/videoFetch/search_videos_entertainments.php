...
$query = mysqli_query($conn,"SELECT * FROM video WHERE  entertainment='{$entertainment}'ORDER BY video_id DESC");
...
$output.=' <span class="text-success" style="font-size:12px;" >'.$description.'</span></span>
   ...
   ';
    ...
    echo $output;
     ...
     $resultuser=mysqli_query($conn,"SELECT * FROM `video` WHERE (title LIKE '%$search%' OR description LIKE '%$search%' OR entertainment LIKE '%$search%' OR username LIKE '%$search%') AND entertainment='{$entertainment}' ");
